...
ANSWER: a = a (any value of type α is equal to itself)
-/
...
ANSWER: 1) reflexive 2) binary 3) one
-/
...
def proof_that_one_eq_one : one_eq_one :=
    -- ANSWER
    --∀ (n : ℕ) one_eq_one n n 
    sorry --had to comment answer on line 58 out since it's incomplete so line 68 was getting an error
/- [10 points]
...
ANSWER: 2 is two more than 0 (rule 1)
...
-- ANSWER: An axiom of this relation is that n = m, so it only works for pairs of the same values
...
Answer:
Symmetric - if x = y, then y = x. In this case, a will always equal a since it is the same
variable, so paramter order does not affect the result. 
Transitive - For x, y, and z: if x is related to y and y is related to z, then x is 
related to z. In this function, if h : (a → a → prop) and (h = id_nat_relation), then 
(id_nat_relation : (a → a → prop))
-/
end hw8